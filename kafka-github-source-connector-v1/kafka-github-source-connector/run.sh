...
if hash docker 2>/dev/null; then
    # for docker lovers
    docker build . -t dhanushka/kafka-github-source-connect-:1.0
    docker run --net=host --rm -t -v $(pwd)/offsets:/kafka-connect-source-github/offsets dhanushka/kafka-github-source-connect:1.0
    ...
    $KAFKA_HOME/bin/connect-standalone.sh $KAFKA_HOME/etc/schema-registry/connect-avro-standalone.properties config/GitHubSourceConnector.properties
